class Nya  {
    /*
    Nya files are zip archives that contains various Quarky assets, styling and scripts(?)
    theme.nya
    | img
    | | | companion...
    | | adminmark.svg...
    ...
    The Nya class implements methods for processing a nya file and
    getting assets from it (Nya.getAsset("img/adminmark.svg") returns file binary)

    Default assets are loaded from decompiled assets ?
    Missing assets are defaulted to default.nya version?

    Is this acceptable? yeåh it myåkes sense
    this ålso måkes sense but i do wonder if it måkes sense (:3)
    does it måke sense to push å zip (nyå) to github
    hm... I wonder if it would be possible to have the "decompiled" nya (current setup) work with Nya (the class)
    probably that is how it should be done
    but also it would be somewhat useful to have a default nya to start modifying for your own theme
    i guess thats just uh.. zip up public
    lol
    myåkes sense
    it would be so much easier to have default.nya :(
    twue :3
    sowwy :3
     */
    /**
     * Loads a nya from a specified URL
     * ```js
     * let nyaFile = await new Nya("https://nya.invalid/theme.nya")
     * ```
     * @param url
     * @returns {Promise<Nya>}
     */
    isDefault = false; // Used to determine if we should be loading assets from public/assets or nya archive
    nya; // JSZip instance
    static defaultInstance; // Instance of Nya with isDefault true
    constructor (url, isDefault = false) {
        this.isDefault = isDefault;
        if (!isDefault) this.init(url)
    }

    async init(url) {
        let nyaZip = new JSZip()
        JSZipUtils.getBinaryContent(url, async (err, data) => {
            if (err) {
                throw err;
            }
            await nyaZip.loadAsync(data)
            this.nya = nyaZip;
            return this
        })
    }
    ...
    /**
     * Gets an asset as an ArrayBuffer from theme or default
     * @param name
     * @returns {Promise<ArrayBuffer>}
     * @throws Error if no file is found
     */
    async getAssetArrayBuffer(name) {
        if (!this.isDefault && this.nya.file(name) !== null) return await this.nya.file(name).async("arraybuffer")
        // If this is the default instance or asset not in theme return array buffer from assets
        try {
            return await JSZipUtils.getBinaryContent(`assets/${name}`);
        } catch (e) {
            e.description = `Failed to load asset ${name}`
            fatalError(e)
        }
    }

    async getAssetDataUrl(name) {
        let arrayBuffer = await this.getAssetArrayBuffer(name);
        let binary = '';
        ...
        let mimetype = await getMimeTypeFromArrayBuffer(arrayBuffer)
        if (mimetype === "text/cache-manifest" && name.endsWith(".svg")) mimetype = "application/svg+xml"
        return `data:${mimetype};base64,${window.btoa( binary )}`;
    }

}

Nya.defaultInstance = new Nya(undefined, true);
...
function getMimeTypeFromArrayBuffer(arrayBuffer) {
    ...
    if (uint8arr.length >= len) {
        ...
        const signature = "0," + signatureArr.join('').toUpperCase()

        let result
        console.log(signature)
        Object.keys(sigDb).forEach(sigName => {
            let sig = sigDb[sigName];
            sig.signs.forEach(sign => {
                if (sign.startsWith(signature)) result = sig.mime;
                return result
            })
        })
        return result
